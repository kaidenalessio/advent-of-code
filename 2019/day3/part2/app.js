...
for (const line1 of wire1) {
	for (const line2 of wire2) {
		...
		if (p instanceof Point) {
			p.steps = [
				line1.step - manhattanDistance(p.x, p.y, line1.p[1].x, line1.p[1].y),
				line2.step - manhattanDistance(p.x, p.y, line2.p[1].x, line2.p[1].y)
			];
...
let fewestCombinedSteps = Number.POSITIVE_INFINITY;

for (const p of intersectsPoints) {
	const combinedSteps = p.steps[0] + p.steps[1];
	if (combinedSteps > 0 && combinedSteps < fewestCombinedSteps) {
		fewestCombinedSteps = combinedSteps;
	}
}

console.log(fewestCombinedSteps);